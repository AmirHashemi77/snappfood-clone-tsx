...
const FoodDetail:FC = () => {
    const currentFood=useSelector((state:RootState)=>state.restrants.currentFood) 
    const cartItems=useSelector((state:RootState)=>state.cart.items)
   
    const availableFood=currentFood && cartItems.find((item)=>item.id===currentFood.id)
    ...
    const plusInCartHandler=()=>{
        if(currentFood){
            dispatch(cartSliceAction.plusHander({
                id:currentFood.id,
    ...
    return (
        <>
            {currentFood && <div className={style.container}>
